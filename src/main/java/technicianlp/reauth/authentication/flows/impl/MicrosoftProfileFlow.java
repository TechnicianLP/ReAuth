...
public final class MicrosoftProfileFlow extends FlowBase {
    ...
    public MicrosoftProfileFlow(Profile profile, FlowCallback callback) {
        ...
        this.profileFuture = this.refreshRequired.thenComposeAsync(Futures.conditional(() -> this.constructProfile(tokens), CompletableFuture.completedFuture(profile)));
        this.profileFuture.thenRun(this::onProfileComplete);
    }
    ...
    private CompletableFuture<Profile> constructProfile(CompletableFuture<Tokens> tokens) {
        CompletableFuture<ProfileEncryption> encryption = this.encryption.thenApplyAsync(ProfileEncryption::randomizedCopy);
